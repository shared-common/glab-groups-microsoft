...
class WorkflowContractTests(unittest.TestCase):
    def test_wrapper_targets_shared_workflow_and_config_path(self) -> None:
        ...
        self.assertIn("config-path: glab-groups-microsoft", text)
        self.assertIn("target-token-secret: GL_PAT_GROUP_MICROSOFT_SVC", text)
        self.assertIn('cron: "5 1 1-31/2 * *"', text)
